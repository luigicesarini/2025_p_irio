...
# print(args) 
entire_region=sf::st_read(glue("../2024_IRIO_EQ/res/Valle d'Aosta_geocoded.gpkg"),quiet=TRUE)


###CARICO Database 
for (year in 1957:2020) {
    print(year)
    lf <- list.files(glue("/mnt/beegfs/lcesarini/2025_p_irio/out/vector/{year}/"),"Aosta",full.name=TRUE)
    if (length(lf) == 0){
        next    
    }else {
        for(fname in lf){
