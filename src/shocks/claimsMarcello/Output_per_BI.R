...
# Add arguments
parser$add_argument("-p", "--path_to_event", type="character" ,help="Path to input file of the flooded event", required=TRUE)
# Parse arguments
...
# path_evento="out/vector/1974/EVENT_40315_1974_Lombardia_River_ul.gpkg"
# /mnt/beegfs/lcesarini/2025_p_irio/out/shocks/claims/1974/BI/EVENT_40315_1974_Lombardia_River_ul.rds
...
entire_region=sf::st_read(glue("../2024_IRIO_EQ/res/{regione}_geocoded.gpkg"),quiet=TRUE)
DTB=sf::st_read(path_evento,quiet=TRUE)

DTB %>% select(Sectors,WD) 
...
if(!"row_id" %in% (entire_region %>% st_set_geometry(NULL) %>% colnames)){
  entire_region <- entire_region %>% rename("row_id"="id") 
}

if(!"row_id" %in% (DTB %>% st_set_geometry(NULL) %>% colnames)){
  DTB <- DTB %>% rename("row_id"="id") 
}
...
left_join(
  ...
  by='row_id') %>% 
  saveRDS(., file = glue::glue('./out/shocks/claims/{year}/BI/{str_replace(basename(path_evento),".gpkg",".rds")}'))
