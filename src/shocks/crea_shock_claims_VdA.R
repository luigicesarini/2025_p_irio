...
# DTB <- readRDS("out/shocks/claims/1959/BI//EVENT_40124_1959_Valle d'Aosta_River_ul.rds")
# DTB  %>% colnames

# all(DTB$irpet_n.x == DTB$irpet_n.y)
# all(DTB$irpet_n.x == DTB$irpet_n)

# all(DTB$ING_sector.x == DTB$ING_sector.y)
# all(DTB$ING_sector.x == DTB$ING_sector)
for (year in 1957:2020) {
    print(year)
    lf <- list.files(glue("/mnt/beegfs/lcesarini/2025_p_irio/out/shocks/claims/{year}/BI/"),"Aosta",full.name=TRUE)
    if (length(lf) == 0){
        next    
    }else {
        for(fname in lf){
            ...
            #SOLO PER alcuni file VALLE D'aosta
            if("irpet_n" %!in% colnames(DTB)){

                DTB <- DTB %>% 
                        select(-irpet_n.x,-ING_sector.x) %>% 
                        rename(
                            irpet_n=irpet_n.y,
                            ING_sector=ING_sector.y
                            )
            }

            if(("irpet_n" %in% colnames(DTB)) & ("irpet_n.x" %in% colnames(DTB))){
